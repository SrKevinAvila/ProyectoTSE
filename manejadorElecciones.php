...
include './clases/Elecciones.php';
include './clases/EleccionesControlador.php';
$alumnoA = new EleccionesControlador();
$accion = $_REQUEST['accion'] ;
switch ($accion) {
	case 'save':
 		if(isset($_REQUEST['bot'])){
 				$alumnoA->setid('NULL');
 				@$alumnoA->setpresidente($_REQUEST['presidente']);
 				@$alumnoA->setalcalde($_REQUEST['alcalde']);
 				@$alumnoA->setdiputado($_REQUEST['diputado']);
 				@$alumnoA->setanio($_REQUEST['anio']);
				$datosObj=array($alumnoA->getid(),
 			 					$alumnoA->getpresidente(),
 								$alumnoA->getalcalde(),
 								$alumnoA->getdiputado(),
								$alumnoA->getanio());
				//var_dump($datosObj);//
			print $alumnoA->guardarDatos($con,$datosObj);
			print '<a href=\'manejadorElecciones.php?accion=con\'>Ver datos</a>';
		}else{
	...
	case 'con':
		...
		print imprimetabla($datoss,"elecciones","table table-bordered",1);
		break;
